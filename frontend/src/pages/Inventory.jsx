...
// Generate random stats for cards that don't have them
function generateStats(tokenId) {
    ...
    return { top: rand(1, 10), right: rand(1, 10), bottom: rand(1, 10), left: rand(1, 10) };
}
    ...
    useEffect(function () {
        ...
        (async function () {
            ...
            try {
                // Load active deck
                var deckRes = null;
                try { deckRes = await apiFetch("/api/decks/active", { token: token }); } catch (e) { }
                if (!alive) return;
                ...
                if (connected && accountId && nftContractId) {
                    console.log("[Inventory] Loading from blockchain:", nftContractId, accountId);
                    try {
                        var nearItems = await nearNftTokensForOwner(nftContractId, accountId);
                        console.log("[Inventory] Got NFTs:", nearItems.length);

                        items = nearItems.map(function (t) {
                            var extra = null;
                            try {
                                if (t.metadata && t.metadata.extra) {
                                    extra = typeof t.metadata.extra === "string"
                                        ? JSON.parse(t.metadata.extra)
                                        : t.metadata.extra;
                                }
                            } catch (e) { }

                            var stats = extra?.stats || generateStats(t.token_id);
                            var imageUrl = t.metadata?.media || "/cards/card.jpg";
                        ...
                        console.warn("[Inventory] blockchain fetch failed:", e);
                    }
                }

                // Fallback to mock NFTs
                if (items.length === 0) {
                    try {
        ...
        <div className="page inventory-page">
            ...
            {connected && accountId && (
                <div className="inv-info-box">
                    <div className="inv-info-label">🔗 Wallet: {accountId.length > 20 ? accountId.slice(0, 10) + "..." + accountId.slice(-6) : accountId}</div>
                    {nftContractId && <div className="inv-info-value">NFT: {nftContractId}</div>}
                    {source && <div className="inv-info-value" style={{ color: source === "blockchain" ? "#22c55e" : "#f59e0b" }}>
                        Source: {source === "blockchain" ? "✅ Blockchain" : "⚠️ Demo cards"}
                    </div>}
                </div>
            )}
            ...
            {loading && <div className="inv-loading"><div className="inv-loading-spinner" /><div>Загрузка NFT… ({nftContractId || "mock"})</div></div>}
            ...
            {!loading && nfts.length === 0 && token && (
                <div className="inv-empty">
                    ...
                    <div className="inv-empty-text">
                        {connected
                            ? "NFT не найдены в " + (nftContractId || "кошельке")
                            : "Подключи кошелёк чтобы увидеть NFT"}
                    </div>
                </div>
            )}
            ...
            {nfts.length > 0 && (
                <div className="inv-grid-game-style">
                    {nfts.map(function (n, idx) {
                        ...
                        return (
                            <button key={k} type="button" onClick={function () { toggle(k); }}
                                ...
                                <div className="inv-card-art-full">
                                    <img src={n.imageUrl || "/cards/card.jpg"} alt={n.name || ""} draggable="false" loading="lazy"
                                        onError={function (e) {
                                            console.warn("[Inventory] Image load failed:", n.imageUrl);
                                ...
                                {isSel && <div className="inv-pick-badge"><div className="inv-pick-badge-inner"><div className="inv-pick-check">✓</div><div className="inv-pick-no">{pickNo}</div></div></div>}
                            </button>
                        );
                    })}
                </div>
            ...
            {nfts.length > 0 && selected.size === 5 && <div className="inv-hint">✅ Колода готова!</div>}
        </div>
    );
}