...
def init_match_state(match_id: str, player1_id: str, player2_id: str,
    ...
    state = {
        ...
        "player1_hand": list(range(5)),  # indices of cards in hand
        "player2_hand": list(range(5)),
        "board": [None] * 9,  # 3x3 grid
        "board_elements": board_elements,
        "current_turn": player1_id,  # who moves now
        "status": "active",
        "winner": None,
        "player1_ready": False,
        "player2_ready": False,
...
def resolve_placement(state: Dict, cell_idx: int, card: Dict, player_id: str) -> List[int]:
    ...
    Returns list of captured cell indices.
    """
    board = state["board"]
    ...
    for neighbor in get_neighbors(cell_idx):
        ...
        # Get values
        my_value = card.get("values", {}).get(my_side, 5)
        their_value = n_card.get("values", {}).get(their_side, 5)
...
def check_game_over(state: Dict) -> Optional[str]:
    ...
    p1_count = sum(1 for cell in board if cell and cell["owner"] == state["player1_id"])
    p2_count = sum(1 for cell in board if cell and cell["owner"] == state["player2_id"])
...
async def broadcast_to_match(match_id: str, message: Dict, exclude_player: str = None):
    ...
    for player_id, ws in match_connections[match_id].items():
...
async def send_game_state(match_id: str, player_id: str = None):
    ...
    if player_id:
        ...
        if match_id in match_connections and player_id in match_connections[match_id]:
            ws = match_connections[match_id][player_id]

            # Add player's hand to message
            ...
            message["your_hand"] = [deck[i] for i in hand_indices]
            message["you_are"] = "player1" if player_id == state["player1_id"] else "player2"
...
async def websocket_match(websocket: WebSocket, match_id: str):
    ...
    try:
        ...
        if player_id and match_id in match_connections:
            ...
            if match_id in match_states and match_states[match_id]["status"] == "active":
            ...
            if not match_connections[match_id]:
